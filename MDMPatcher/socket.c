...
void socket_set_verbose(int level) { verbose = level; }
...
#ifdef SO_NOSIGPIPE
  if (setsockopt(sock, SOL_SOCKET, SO_NOSIGPIPE, (void *)&yes, sizeof(int)) ==
      -1) {
    perror("setsockopt()");
    socket_close(sock);
    return -1;
...
static int32_t _sockaddr_in6_scope_id(struct sockaddr_in6 *addr) {
  ...
  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
    ...
    if (memcmp(&addr->sin6_addr.s6_addr, &addr_in->sin6_addr.s6_addr,
               sizeof(addr_in->sin6_addr.s6_addr)) == 0) {
...
int socket_shutdown(int fd, int how) { return shutdown(fd, how); }
