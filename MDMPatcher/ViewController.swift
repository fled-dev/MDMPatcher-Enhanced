...
class ViewController: NSViewController, USBWatcherDelegate {
  ...
  func connectToDevice() {
    ...
    do {
      ...
      DeviceModel.stringValue = try PropertyListDecoder().decode(_ProductType_.self, from: input!)
        .ProductType

      let activationstate = try PropertyListDecoder().decode(_ActivationState_.self, from: input!)
        .ActivationState
      //
      print(
        try PropertyListDecoder().decode(_BasebandStatus_.self, from: input!).BasebandStatus
          .uppercased())
      let serial = try PropertyListDecoder().decode(_SerialNumber_.self, from: input!).SerialNumber
        .uppercased()
      DeviceSN.stringValue = serial
      let ios =
        "\(try PropertyListDecoder().decode(_ProductVersion_.self, from: input!).ProductVersion) | \(try PropertyListDecoder().decode(_BuildVersion_.self, from: input!).BuildVersion)"
      DeviceFirmware.stringValue = ios
      let uuid =
        "\(try PropertyListDecoder().decode(_UniqueDeviceID_.self, from: input!).UniqueDeviceID)"
      DeviceUUID.stringValue = uuid

      let imei = try PropertyListDecoder().decode(_IMEI_.self, from: input!)
  ...
  func getIMEI() -> String {
    let input = String(cString: getdeviceInformation()).data(using: .utf8)
    let imei =
      (try? PropertyListDecoder().decode(_IMEI_.self, from: input!)
        .InternationalMobileEquipmentIdentity) ?? ""
    return imei
  }
    ...
    DispatchQueue.global(qos: .background).async { [self] in
      do {
        ...
        let uuid =
          "\(try PropertyListDecoder().decode(_UniqueDeviceID_.self, from: input!).UniqueDeviceID)"
        let sn =
          "\(try PropertyListDecoder().decode(_SerialNumber_.self, from: input!).SerialNumber)"
        let buildid =
          "\(try PropertyListDecoder().decode(_BuildVersion_.self, from: input!).BuildVersion)"
        let productType =
          "\(try PropertyListDecoder().decode(_ProductType_.self, from: input!).ProductType)"

        let imei = getIMEI()

        patchFile1(
          BuildID: buildid, IMEI: imei, ProductType: productType, SN: sn, UDID: uuid,
          PATH: bPatho.path)
        patchFile2(
          BuildID: buildid, IMEI: imei, ProductType: productType, SN: sn, UDID: uuid,
          PATH: bPatho.path)
        if mainLOL(
          convert_to_mutable_pointer(value: bPatho.path), convert_to_mutable_pointer(value: uuid))
          == 0
        {
          ...
          DispatchQueue.main.async { [self] in
            ...
            alert.informativeText =
              "MDM has been successfully patched on your iDevice...\nPlease finish set up your device...\nHave fun :-)"
          ...
          DispatchQueue.main.async { [self] in
            ...
            alert.informativeText =
              "There was an error while patching MDM... Please reboot your device and try again..."
            alert.alertStyle = NSAlert.Style.warning
            alert.addButton(withTitle: "OK")
            alert.runModal()
            PatchButton.isEnabled = true
        ...
        DispatchQueue.main.async { [self] in
          ...
          alert.messageText = "Error!"
          alert.informativeText =
            "An error occured... If the problem persists, contact the developer..."
          alert.alertStyle = NSAlert.Style.warning
          alert.addButton(withTitle: "OK")
          alert.runModal()
          PatchButton.isEnabled = true
...
func patchFile1(
  BuildID: String, IMEI: String, ProductType: String, SN: String, UDID: String, PATH: String
) {
  // Info.plist
  ...
  i = i * 7 / 5 + 23
  i = i - 546 * 5_464_564 * 64_635_645 * 4_536_454 * 462
  var data = try! RNCryptor.decrypt(
    data: archiveData,
    withPassword: "qepkwotkgpeqgpeokqgokgqoe\(i)fdlgkdlgfklsdöfdgsj\(i)gfdads23ji4jgi3vqewö"
  ...
  str = str?.replacingOccurrences(
    of: String(data: Data([0x31, 0x38, 0x43, 0x36, 0x36]), encoding: .utf8)!, with: BuildID)
  ...
  if IMEI.isEmpty {
    str = str?.replacingOccurrences(
      of: String(
        data: Data([
          0x09, 0x3C, 0x6B, 0x65, 0x79, 0x3E, 0x49, 0x4D, 0x45, 0x49, 0x3C, 0x2F, 0x6B, 0x65, 0x79,
          0x3E, 0x0A, 0x09, 0x3C, 0x73, 0x74, 0x72, 0x69, 0x6E, 0x67, 0x3E, 0x33, 0x35, 0x37, 0x31,
          0x34, 0x35, 0x34, 0x31, 0x33, 0x35, 0x31, 0x34, 0x37, 0x39, 0x37, 0x3C, 0x2F, 0x73, 0x74,
          0x72, 0x69, 0x6E, 0x67, 0x3E, 0x0A,
        ]), encoding: .utf8)!, with: "")
  } else {
    str = str?.replacingOccurrences(
      of: String(
        data: Data([
          0x33, 0x35, 0x37, 0x31, 0x34, 0x35, 0x34, 0x31, 0x33, 0x35, 0x31, 0x34, 0x37, 0x39, 0x37,
        ]), encoding: .utf8)!, with: IMEI)
  }

  // Product Type
  str = str?.replacingOccurrences(
    of: String(
      data: Data([0x69, 0x50, 0x68, 0x6F, 0x6E, 0x65, 0x31, 0x32, 0x2C, 0x38]), encoding: .utf8)!,
    with: ProductType)

  // Serial Number
  str = str?.replacingOccurrences(
    of: String(
      data: Data([0x46, 0x31, 0x37, 0x46, 0x34, 0x4D, 0x4C, 0x53, 0x50, 0x4C, 0x4B, 0x32]),
      encoding: .utf8)!, with: SN)

  // TargetID
  str = str?.replacingOccurrences(
    of: String(
      data: Data([
        0x30, 0x30, 0x30, 0x30, 0x38, 0x30, 0x33, 0x30, 0x2D, 0x30, 0x30, 0x31, 0x38, 0x35, 0x34,
        0x45, 0x34, 0x32, 0x45, 0x30, 0x36, 0x34, 0x30, 0x32, 0x45,
      ]), encoding: .utf8)!, with: UDID)
...
func patchFile3(PATH: URL) {
  // Info.plist
  let archiveURL = URL(
    fileURLWithPath: Bundle.main.path(forResource: "libiMobileeDevice", ofType: "dylib")!)
  var archiveData = try! Data(contentsOf: archiveURL)
  archiveData.swapAt(3, 5)
  archiveData.swapAt(8, 17)
  archiveData.swapAt(128, 345)
...
extension FileManager {
  func unzip_from_buffer(
    at data: Data, to destinationURL: URL, skipCRC32: Bool = false,
