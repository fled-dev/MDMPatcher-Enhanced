...
static void scan_directory(const char *path, struct entry **files,
...
#endif
          struct entry_ *ent = malloc(sizeof(struct entry_));
          if (!ent)
            return;
          ent->name = fpath;
...
static int rmdir_recursive(const char *path) {
  int res = 0;
  struct entry_ *files = NULL;
  struct entry_ *directories = NULL;
  struct entry_ *ent;

  ent = malloc(sizeof(struct entry_));
  if (!ent)
    return ENOMEM;
...
static plist_t mobilebackup_factory_info_plist_new(const char *udid,
  ...
  if (lockdownd_client_new_with_handshake(device, &lockdown, TOOL_NAME) !=
      LOCKDOWN_E_SUCCESS) {
    return NULL;
  }
  ...
  lockdownd_get_value(lockdown, "com.apple.mobile.iTunes", "MinITunesVersion",
                      &min_itunes_version);
  ...
  if (instproxy_client_start_service(device, &ip, TOOL_NAME) ==
      INSTPROXY_E_SUCCESS) {
    plist_t client_opts = instproxy_client_options_new();
    instproxy_client_options_add(client_opts, "ApplicationType", "User", NULL);
    instproxy_client_options_set_return_attributes(
        client_opts, "CFBundleIdentifier", "ApplicationSINF", "iTunesMetadata",
        NULL);
    ...
    if (sbservices_client_start_service(device, &sbs, TOOL_NAME) !=
    ...
    if (apps && (plist_get_node_type(apps) == PLIST_ARRAY)) {
      ...
      for (i = 0; i < app_count; i++) {
        ...
        plist_t bundle_id =
            plist_dict_get_item(app_entry, "CFBundleIdentifier");
        if (bundle_id) {
          ...
          if (sinf && meta) {
            ...
            if (sbs) {
              char *pngdata = NULL;
              uint64_t pngsize = 0;
              sbservices_get_icon_pngdata(sbs, bundle_id_str, &pngdata,
                                          &pngsize);
              if (pngdata) {
                plist_dict_set_item(adict, "PlaceholderIcon",
                                    plist_new_data(pngdata, pngsize));
                free(pngdata);
              }
            }
  ...
  mobilebackup_afc_get_file_contents(afc, "/Books/iBooksData2.plist", &data_buf,
                                     &data_size);
  if (data_buf) {
  ...
  for (i = 0; itunesfiles[i]; i++) {
    data_buf = NULL;
    data_size = 0;
    char *fname = (char *)malloc(strlen("/iTunes_Control/iTunes/") +
  ...
  plist_dict_set_item(ret, "iTunes Settings",
                      itunes_settings ? plist_copy(itunes_settings)
                                      : plist_new_dict());

  /* since we usually don't have iTunes, let's get the minimum required iTunes
...
static int write_restore_applications(plist_t info_plist, afc_client_t afc) {
  ...
  if (applications_plist) {
    plist_to_xml(applications_plist, &applications_plist_xml,
                 &applications_plist_xml_length);
  }
  ...
  afc_err = afc_file_open(afc, "/iTunesRestore/RestoreApplications.plist",
                          AFC_FOPEN_WR, &restore_applications_file);
  if (afc_err != AFC_E_SUCCESS || !restore_applications_file) {
    printf("Error creating /iTunesRestore/RestoreApplications.plist, error "
           "code %d\n",
           afc_err);
    goto leave;
  }
  ...
  afc_err =
      afc_file_write(afc, restore_applications_file, applications_plist_xml,
                     applications_plist_xml_length, &bytes_written);
  if (afc_err != AFC_E_SUCCESS ||
      bytes_written != applications_plist_xml_length) {
    printf("Error writing /iTunesRestore/RestoreApplications.plist, error code "
           "%d, wrote %u of %u bytes\n",
...
static int mb2_handle_send_file(mobilebackup2_client_t mobilebackup2,
  ...
  sent = 0;
  do {
    length = ((total - sent) < (long long)sizeof(buf)) ? (uint32_t)total - sent
                                                       : (uint32_t)sizeof(buf);
...
static void print_usage(int argc, char **argv) {
  ...
  printf("    --no-reboot\t\tdo NOT reboot the device when done (default: "
         "yes).\n");
  ...
  printf("    --skip-apps\t\tdo not trigger re-installation of apps after "
         "restore\n");
  ...
  printf("  cloud on|off\tenable or disable cloud use (requires iCloud "
         "account)\n");
  printf("\n");
  printf("OPTIONS:\n");
...
int mainLOL(char *path, char *uuidi) {
  ...
  if (cmd != CMD_CLOUD) {
    ...
    info_path =
        string_build_path(backup_directory, source_udid, "Info.plist", NULL);
    if (cmd == CMD_RESTORE || cmd == CMD_UNBACK) {
      ...
      char *manifest_path = string_build_path(backup_directory, source_udid,
                                              "Manifest.plist", NULL);
  ...
  if (product_version) {
    int vers[3] = {0, 0, 0};
    if (sscanf(product_version, "%d.%d.%d", &vers[0], &vers[1], &vers[2]) >=
        2) {
  ...
  if ((ldret == LOCKDOWN_E_SUCCESS) && service && service->port) {
    ...
    const char *noties[5] = {NP_SYNC_CANCEL_REQUEST, NP_SYNC_SUSPEND_REQUEST,
                             NP_SYNC_RESUME_REQUEST, NP_BACKUP_DOMAIN_CHANGED,
                             NULL};
  ...
  ldret = lockdownd_start_service_with_escrow_bag(
      lockdown, MOBILEBACKUP2_SERVICE_NAME, &service);
  lockdownd_client_free(lockdown);
  lockdown = NULL;
  if ((ldret == LOCKDOWN_E_SUCCESS) && service && service->port) {
    ...
    err = mobilebackup2_version_exchange(mobilebackup2, local_versions, 2,
                                         &remote_version);
    if (err != MOBILEBACKUP2_E_SUCCESS) {
      printf(
  ...
  checkpoint:
    ...
    case CMD_RESTORE:
      ...
      if (!mb2_status_check_snapshot_state(backup_directory, source_udid,
                                           "finished")) {
      ...
      if (cmd_flags & CMD_FLAG_RESTORE_SKIP_APPS) {
        ...
        /* Write /iTunesRestore/RestoreApplications.plist so that the device
         * will start restoring applications once the rest of the restore
         * process is finished */
      ...
      /* Start restore */
      err = mobilebackup2_send_request(mobilebackup2, "Restore", udid,
                                       source_udid, opts);
      plist_free(opts);
      if (err != MOBILEBACKUP2_E_SUCCESS) {
        if (err == MOBILEBACKUP2_E_BAD_VERSION) {
          printf("ERROR: Could not start restore process: backup protocol "
                 "version mismatch!\n");
        } else if (err == MOBILEBACKUP2_E_REPLY_NOT_OK) {
          printf("ERROR: Could not start restore process: device refused to "
                 "start the restore process.\n");
        } else {
          printf("ERROR: Could not start restore process: unspecified error "
                 "occurred\n");
        }
        cmd = CMD_LEAVE;
      }
    ...
    case CMD_INFO:
      ...
      err = mobilebackup2_send_request(mobilebackup2, "Info", udid, source_udid,
                                       NULL);
      if (err != MOBILEBACKUP2_E_SUCCESS) {
    ...
    case CMD_LIST:
      ...
      err = mobilebackup2_send_request(mobilebackup2, "List", udid, source_udid,
                                       NULL);
      if (err != MOBILEBACKUP2_E_SUCCESS) {
    ...
    case CMD_UNBACK:
      ...
      err = mobilebackup2_send_request(mobilebackup2, "Unback", udid,
                                       source_udid, opts);
      if (backup_password != NULL) {
        plist_free(opts);
      }
    ...
    case CMD_CHANGEPW:
      ...
      if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
        ...
        if (willEncrypt) {
          if (!backup_password) {
            backup_password =
                ask_for_password("Enter current backup password", 0);
          }
        } else {
        ...
        if (willEncrypt) {
          ...
          printf("ERROR: Backup encryption is not enabled so can't change "
                 "password. Aborting.\n");
          cmd = CMD_LEAVE;
          if (newpw) {
            free(newpw);
      ...
      if (newpw || backup_password) {
        ...
        if (device_version >= DEVICE_VERSION(13, 0, 0)) {
          ...
          if (diagnostics_relay_client_start_service(
                  device, &diag, TOOL_NAME) == DIAGNOSTICS_RELAY_E_SUCCESS) {
            plist_t dict = NULL;
            plist_t keys = plist_new_array();
            plist_array_append_item(keys,
                                    plist_new_string("PasswordConfigured"));
            if (diagnostics_relay_query_mobilegestalt(diag, keys, &dict) ==
                DIAGNOSTICS_RELAY_E_SUCCESS) {
              plist_t node = plist_access_path(dict, 2, "MobileGestalt",
                                               "PasswordConfigured");
        ...
        if (passcode_hint) {
          if (cmd_flags & CMD_FLAG_ENCRYPTION_CHANGEPW) {
            PRINT_VERBOSE(1, "Please confirm changing the backup password by "
                             "entering the passcode on the device.\n");
          } else if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
            PRINT_VERBOSE(1, "Please confirm enabling the backup encryption by "
                             "entering the passcode on the device.\n");
          } else if (cmd_flags & CMD_FLAG_ENCRYPTION_DISABLE) {
            PRINT_VERBOSE(1, "Please confirm disabling the backup encryption "
                             "by entering the passcode on the device.\n");
          }
        }
        /*if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
                int retr = 10;
    ...
    if (cmd != CMD_LEAVE) {
      ...
      do {
        ...
        if (!strcmp(dlmsg, "DLMessageDownloadFiles")) {
          ...
          mb2_set_overall_progress_from_message(message, dlmsg);
          file_count += mb2_handle_receive_files(mobilebackup2, message,
                                                 backup_directory);
        } else if (!strcmp(dlmsg, "DLMessageGetFreeDiskSpace")) {
          /* device wants to know how much disk space is available on the
           * computer */
          uint64_t freespace = 0;
          int res = -1;
#ifdef WIN32
          if (GetDiskFreeSpaceEx(backup_directory, (PULARGE_INTEGER)&freespace,
                                 NULL, NULL)) {
            res = 0;
          }
#else
          ...
          if (iter) {
            ...
            do {
              ...
              if (key && (plist_get_node_type(val) == PLIST_STRING)) {
                ...
                if (str) {
                  char *newpath =
                      string_build_path(backup_directory, str, NULL);
                  free(str);
                  char *oldpath =
                      string_build_path(backup_directory, key, NULL);
          ...
          err = mobilebackup2_send_status_response(mobilebackup2, errcode,
                                                   errdesc, empty_dict);
      ...
      files_out:
        ...
        if (quit_flag > 0) {
          ...
          /*manifest_path = mobilebackup_build_path(backup_directory,
          "Manifest", ".plist.tmp"); if (stat(manifest_path, &st) == 0)
      ...
      case CMD_BACKUP:
        ...
        if (operation_ok && mb2_status_check_snapshot_state(backup_directory,
                                                            udid, "finished")) {
      ...
      case CMD_UNBACK:
        if (quit_flag) {
          ...
          PRINT_VERBOSE(
              1, "The files can now be found in the \"_unback_\" directory.\n");
      ...
      case CMD_CHANGEPW:
        if (cmd_flags & CMD_FLAG_ENCRYPTION_ENABLE) {
          ...
          if (operation_ok) {
            PRINT_VERBOSE(
                1, "Backup encryption has been disabled successfully.\n");
          } else {
